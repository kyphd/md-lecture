...
unit_pressure = unit_eng * 1.602e-19 / (unit_length * 1e-10) ** 2 / 1e-10     # Pressure (Pa) for 2D (N/m)
...
Tconst = 100        # Constant Temperature [K]
rcut_ = 3.0         # Cut off (Nondimension)

# Nondimensionalization
m_ = 1.0                                # Mass
Tconst_ = Tconst / unit_temp            # Temperature
...
# velocity scaling
def velocity_scaling(T_, KE_, vx_, vy_):
    dim = 2
    a = np.sqrt(dim * Natom * T_ / (2 * KE_))
    vx_ *= a
    vy_ *= a
...
def nvt():
    ...
    while n_step <= Max_step:
        ...
        if n_step == 0:
            ...
            initialize_pos_vel(x_, y_, vx_, vy_, Tconst_)
            KE_ = (np.sum(vx_ ** 2) + np.sum(vy_ ** 2)) / 2
            velocity_scaling(Tconst_, KE_, vx_, vy_)            # velocity scaling
            ...
            # velocity scaling
            KE_ = (np.sum(vx_ ** 2) + np.sum(vy_ ** 2)) / 2
            velocity_scaling(Tconst_, KE_, vx_, vy_)
